...
object ColorMatrixEngine {
   ...
   * and selected 2027 color grading LUT/presets through direct matrix mathematics.
  ...
  fun createUnifiedMatrix(state: AdjustmentsState): ColorMatrix {
    ...
    when (state.selectedPreset) {
      ...
      FilterPreset.CYBERPUNK -> {
        // 2027 Neo-Tokyo: saturated neon cyan, magenta highlights, ultra high contrast
        r1 *= 1.18f
        g2 *= 0.90f
        b3 *= 1.45f
        offR += 22f
